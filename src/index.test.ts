...
describe("Todo API & DevOps Health Check (Bun Native)", () => {
  ...
  it("POST /api/todos harus membuat todo baru", async () => {
    const res = await fetch(`${BASE_URL}/api/todos`, {
      ...
      body: JSON.stringify({ title: "Belajar Docker & CI/CD Native" }),
    });

    expect(res.status).toBe(201);
    const body = await res.json();
    expect(body.success).toBe(true);
    expect(body.data.title).toBe("Belajar Docker & CI/CD Native");
